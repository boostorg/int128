...
When a `uint128` is compared with an `int128`, the operators convert the signed operand to unsigned, so `pass:[int128{-1} < uint128{0}]` is reported as `false` and `pass:[(std::numeric_limits<uint128>::max)() == int128{-1}]` as `true`.
